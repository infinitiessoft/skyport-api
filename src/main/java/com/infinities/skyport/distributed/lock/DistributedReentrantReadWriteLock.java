...
public class DistributedReentrantReadWriteLock implements ReadWriteLock {
	...
	static class LockImpl {
		...
		void acquireShared() throws InterruptedException {
			...
			try {
				if (holds.get().count > 0) {
					...
					// the checking of these variables is guarded by the monitor
					// which all methods acquire
		 ...
		 * @param timeUnit
		 *            timeout units
		 * @return true if the exclusive lock was acquired; false o/w or if
		 *         timeout was <= 0
		 * @throws InterruptedException
	...
	public static class WriteLock implements Lock {
		...
		/**
		 * @return the number of local threads holding the write lock (should
		 *         only ever be 0 or 1)
		 */
		public int getHoldCount() {
